...
definition(
    name: "Home Management SmartApp v2",
...
def morningPage() {
    dynamicPage(name: "morningPage", title: "At Morning Time", nextPage: "notificationPage", install: false, uninstall: false) {
		section("Select the time to execute"){
        	input "morningTime", "time", required: true, default: 0
        }
		// get the available actions
            def actions = location.helloHome?.getPhrases()*.label
            if (actions) {
...
def arrivalHandler(evt)
{
    ...
    if (firstOneHome()) {
        ...
        message = "First One Home! ${alarmMsg} / Mode: ${modeMsg}"
...
/*
        ...
        message = "Not The First One Home!"
        if (sendPush || arrivalPush) {
            sendPush(message)
        }
        if (phone) {
...
def departureHandler(evt)
{
	...
	if (everyoneIsAway()) {
        ...
        message = "Performing Goodbye Home! ${alarmMsg} / Mode: ${modeMsg}"
...
def runEveningSchedule(evt) {
	...
	if (presenceValue) { 
        ...
        message = "Someone is home in the evening: ${alarmMsg} / Mode: ${modeMsg}"
        ...
        message = "Everyone is away in the evening: ${alarmMsg} / Mode: ${modeMsg}"
...
def morningHandler(evt) {
	...
	if (presenceValue) { 
        ...
        message = "Someone is home in the morning: ${alarmMsg} / Mode: ${modeMsg}"
        ...
        message = "Everyone is away in the morning: ${alarmMsg} / Mode: ${modeMsg}"
...
def nightHandler(evt) {
	...
	if (presenceValue) { 
        ...
        message = "Someone is home at night: ${alarmMsg} / Mode: ${modeMsg}"
        ...
        message = "Everyone is away at night: ${alarmMsg} / Mode: ${modeMsg}"
