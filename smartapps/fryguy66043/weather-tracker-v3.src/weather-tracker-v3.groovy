...
//    sendSms("9136831550", "getLux(${wxIcon})")
	...
	if (timeOfDayIsBetween(daylight.sunrise, daylight.sunset, new Date(), location.timeZone)) {
        ...
        def oneAndAHalfHour = oneHour + halfHour
        def nowTime = new Date().time
        def sunriseTime = daylight.sunrise.time - halfHour
        def sunsetTime = daylight.sunset.time + halfHour
        def afterSunrise = nowTime - sunriseTime 
        def beforeSunset = sunsetTime - nowTime

//        if(afterSunrise < oneHour) {
        if(afterSunrise < oneAndAHalfHour) {
            //dawn
//            lux = (long)(lux * (afterSunrise/oneHour))
            lux = (long)(lux * (afterSunrise/oneAndAHalfHour))
//        } else if (beforeSunset < oneHour) {
        } else if (beforeSunset < oneAndAHalfHour) {
            //dusk
//            lux = (long)(lux * (beforeSunset/oneHour))
            lux = (long)(lux * (beforeSunset/oneAndAHalfHour))
        }        
    }
    else {
